...
const io = new Server(server, {
    ...
    maxHttpBufferSize: 1e8 // Permet de gérer des fichiers jusqu'à 100MB
});
...
// ----- Upload fichiers (Inutilisé avec Socket.IO, mais gardé pour référence) -----
...
io.on("connection", (socket) => {
    ...
    // NOUVEL ÉVÉNEMENT POUR L'UPLOAD DE FICHIERS
    socket.on("file-upload", (data) => {
        // Validation basique côté serveur
        if (!data.fileData || !data.fileName || !data.fileType || !data.room) {
            socket.emit("notification", "Fichier invalide.");
            return;
        }

        // Diffuse le fichier à tous les clients du salon
        io.to(data.room).emit("message", {
            user: data.user,
    ...
    function broadcastUserList(room) {
        const clients = Array.from(io.sockets.adapter.rooms.get(room) || []).map(id => {
            ...
            // Je n'ai pas la variable `username` sur le socket.
            // On peut la définir lors du "joinRoom" pour que ça marche.
            // Par exemple : socket.username = username;
            // Pour l'instant, on se base sur `onlineUsers`.
            return onlineUsers.get(id) || "Anonyme";
        });
